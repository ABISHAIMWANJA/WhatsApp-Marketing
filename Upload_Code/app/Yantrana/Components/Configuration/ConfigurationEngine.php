    ...
    public function processConfigurationsStore($pageType, $inputData, $ignoreOtherFields = false)
    {
        ...
        if (! __isEmpty($inputData)) {
            ...
            foreach ($defaultSettings as $defaultInputKey => $defaultInputValue) {
                ...
                if ($inputKey == 'lwEnableWhatsJetRazorpaySubscriptionAddon' 
                    and $inputValue == 1 
                    and getAppSettings('enable_razorpay_subscription')
                    and getAppSettings('enable_stripe')
                ) {
                    return $this->engineResponse(21, [
                        'show_message' => true,
                        'messageType' => 'error',
                        'reloadPage' => true
                    ], __tr(configItem('razorpay_enable_warning_message'), [
                        '__firstPaymentGatewayName__' => 'Razorpay',
                        '__SecondPaymentGatewayName__' => 'Stripe',
                    ]));

                }

                if ($inputKey == 'enable_stripe' and $inputValue == 'on' and getAppSettings('enable_razorpay_subscription') and getAppSettings('lwEnableWhatsJetRazorpaySubscriptionAddon')) {
                    return $this->engineFailedResponse([
                        'show_message' => true
                    ], __tr(configItem('razorpay_enable_warning_message'), [
                        '__firstPaymentGatewayName__' => 'Stripe',
                        '__SecondPaymentGatewayName__' => 'Razorpay',
                    ]));
                }
    ...
    public function processProductRegistration($inputData = [])
    {
        return $this->processConfigurationsStore('product_registration', [
            'product_registration' => [
                ...
                'sold_at' => array_get($inputData, 'sold_at', ''),
                'registered_at' => now(),
                'signature' => hash('sha256',
                    array_get($_SERVER, 'HTTP_HOST', '') .
                        array_get($inputData, 'registration_id', '') . config('lwSystem.version')
                ),
            ],
        ]);
    }
